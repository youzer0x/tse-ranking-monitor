...
1. `disclosures`：入力の `disclosures`（前営業日15:30以降〜当日15:30前のTDnet開示）が値動きを説明するときだけ `factor_kind="開示"` とする。
...
4. `news.prior`：`disclosures`・`news.material_window` に新規材料がない場合に限り、起点日を明記した継続テーマとして使う。
...
`factor` はランキング表にそのまま表示される。要因の本質だけを凝縮し、冗長性を排除する。

- **表示250字以内**（Markdownリンクはラベルだけを数え、URLは数えない。開示タグの行に自動で付く `[開示PDF]` も数えない）。目安は200字前後とし、上限に余裕を残す。裏付けの詳細は `claims`、市場分析向けの要約は `market_note` に書き、`factor` には「何が・いつ・なぜ株価を動かしたか」だけを書く。
- 書き出しを銘柄名の主語（「〇〇は」）にしない。当該銘柄の記載であることは自明である。
- 誰でも知っている背景（休場日・連休・曜日の並び、「開示後最初の取引日だった」等）を書かない。
- 業種コード・クラスタID（`s33:3650` 等）・入力のフィールド名（`pct5`・`sec33`・`leader_code`・`material_window` 等）を書かない。業種に触れるときは業種名だけを書く。
