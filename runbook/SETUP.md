...
東証 日中（レギュラー）値上がり率ランキングを**日次・無人**で生成し、**GitHub Pages（Web）＋ Gmail（API）**で配信するための手順。PTS 版 `pts-ranking-monitor` と**同じ構成・同じ Gmail 方式・Claude のスケジュール（ルーチン）機能**で運用する。方法論は `vendor/tse-ranking-digest/SKILL.md`、ルーチン仕様はリポジトリルートの `AGENTS.md`、貼り付けプロンプトは同じ `runbook/` 内の `ROUTINE_PROMPT.md`。
...
本ディレクトリには `src/tse_ranking_monitor/` の本体実装と、従来の実行方法を維持する `scripts/*.py` 互換CLIが同梱済みである。データ取得系の共有スクリプト（`jquants.py`・`business_day.py`・`kabutan_pts.py`・`tdnet.py`・`market_cap_jquants.py`・`market_cap_yahoo.py`）は共有リポ **`market-scripts-common`** が単一の真実源で、同リポの sync.py が配布と `scripts/vendor.lock.json` の刻印を行う（手動 `cp` はしない）：
...
tse-ranking-monitor/
├── README.md                 # 入口（概要・仕組み）
├── AGENTS.md                 # 必読順・停止条件・SOT境界
├── CLAUDE.md                 # 対話開発と検証の規範
├── requirements.txt
├── src/tse_ranking_monitor/  # 本リポ固有の実装本体
├── scripts/
│   ├── build_day_ranking.py  # Stage1の既存CLI互換入口
...
│   ├── build_market_stats.py / build_market_json.py                # 市場分析の互換入口
...
├── vendor/tse-ranking-digest/ # 方法論の版固定スナップショット
├── tools/                     # vendor整合性チェック
├── runbook/                   # 日次手順・本手順書・貼り付けプロンプト
├── specs/MARKET_ANALYSIS.md   # 市場分析固有の仕様
├── docs/                      # GitHub Pages公開成果物のみ
└── .work/<SESSION>/           # 作業中ファイル（gitignore・非公開）
```
...
git status
git add . && git commit -m "Initial commit: TSE day ranking monitor"
git branch -M main
git remote add origin https://github.com/<あなた>/tse-ranking-monitor.git  # origin未設定時のみ
git push -u origin main
```
> 本ディレクトリは独立リポジトリである。既存の `.git/` と履歴を維持し、再度 `git init` しない。
...
Step 8 と**同一の環境・プロンプト・モデル・ツール・MCP**で、cron だけを変えた2本目のルーチンを作る。

- **スケジュール（cron）**：毎日 **20:35 JST**（UTC 指定なら `35 11 * * *`）
- 名前は一次と区別できるものにする（例：`東証 値上がり率ランキング（バックアップ 20:35）`）
- 通知は一次と同じくプッシュのみ有効にする

> **なぜ2本必要か**：一次ルーチンが Stage2〜市場分析の途中でクラウドセッションごと落ちると、`RUNTIME_CONTRACT.md` §6 の失敗通知も走らない。§6 はエージェントへの指示であってコード上の強制点が無く、テレメトリの出力先も gitignore された `.work/` なので、**セッションが死ぬと通知もログも残らず無音で欠落する**。`watchdog.yml` は `contents: read`／`issues: write` しか持たず**再実行できない**（検知して issue を立てるだけ・メールも送らない）。したがって2本目の発火だけがセッション死に対する実効的な復旧手段である。
