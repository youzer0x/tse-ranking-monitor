...
> **なぜ2本必要か**：一次ルーチンが Stage2〜市場分析の途中でクラウドセッションごと落ちると配信が欠落する。`watchdog.yml` は `contents: read`／`issues: write` しか持たず**再実行できない**（検知するだけ）。したがって2本目の発火だけがセッション死に対する実効的な**復旧**手段である。**検知**は別layer（終了時ガード＋耐久ステータス＋watchdog）が担う。`DAILY_ROUTINE.md` の「監視と失敗通知」を参照。
