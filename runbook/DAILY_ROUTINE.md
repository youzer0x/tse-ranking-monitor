...
- **factor_kind の空欄・既定外を残さない（機械検査＝WARN）**：空欄・`確認不可` 等は不可。材料を特定できない行も
  5パス確認後に `status=unresolved`・`[テーマ]` とし（5パスの実施は evidence の `checks` が記録する）、本文は空欄にせず
  値動きの最も有力な理由を推定表現で書く（監査 §4 の空欄・確認不可の解消）。
- **factor は表示250字以内で要因の本質に絞る（機械検査＝WARN・2026-09-24 配信の編集レビュー由来）**：字数はMarkdown
  リンクをラベルだけ数える（URL・自動付与の `[開示PDF]` は数えない）。次を書かない：①書き出しの銘柄名主語（「〇〇は」）
  ②誰でも知っている背景（休場日・連休・曜日の並び等）③業種コード・クラスタID（`s33:3650`）・入力フィールド名（`pct5` 等）
  ④一般的でない用語「材料窓」「窓内」「窓外」（時点は日付・時刻で書く）⑤「適時開示なし」「材料は確認できず」等の
