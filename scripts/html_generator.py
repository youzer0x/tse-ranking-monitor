...
function themeSection(tm){
  tm=tm||{};
  if(!asArr(tm.rows).length)return '';
  ...
  asArr(tm.rows).forEach(function(r){h+='<tr><td class="thead-note">'+esc(r.theme)+'</td><td class="mbuy">'+mdInline(r.bought||'')+'</td><td class="msell">'+mdInline(r.sold||'')+'</td></tr>';});
  h+='</tbody></table></div>';
